...
public interface OptimizedCredentialRepository extends JpaRepository<Credential, Long> {
     ...
     */
    @Query("SELECT c FROM Credential c WHERE c.userId = :userId AND c.isDeleted = false ORDER BY c.updatedAt DESC")
     ...
     */
    @Query("SELECT c FROM Credential c WHERE c.credentialId = :credentialId AND c.userId = :userId AND c.isDeleted = false")
     ...
     */
    @Query("SELECT c FROM Credential c WHERE c.userId = :userId AND c.category = :category AND c.isDeleted = false ORDER BY c.serviceName ASC")
     ...
     */
    @Query("SELECT COUNT(c) FROM Credential c WHERE c.userId = :userId AND c.isDeleted = false")
    ...
    @Query("""
        UPDATE Credential c 
        SET c.isDeleted = true, c.updatedAt = :deleteTime 
    ...
    @Query("""
        UPDATE Credential c 
        SET c.isDeleted = false, c.updatedAt = :restoreTime 
        WHERE c.userId = :userId AND c.credentialId IN :credentialIds AND c.isDeleted = true
        """)
    ...
    @Query("""
        ...
        AND c.isDeleted = false
        AND (c.userId = :userId OR (cs.sharedWithUserId = :userId AND cs.isActive = true))
        """)
