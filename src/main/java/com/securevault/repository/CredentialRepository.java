...
public interface CredentialRepository extends JpaRepository<Credential, Long>, JpaSpecificationExecutor<Credential> {
    ...
    /**
     * Find credential by ID and user ID including soft-deleted credentials
     * Used for restore and permanent delete operations
     * 
     * @param credentialId the ID of the credential
     * @param userId the ID of the user
     * @return Optional containing the credential if found
     */
    @Query("SELECT c FROM Credential c WHERE c.credentialId = :credentialId AND c.userId = :userId")
    Optional<Credential> findByCredentialIdAndUserIdIncludingDeleted(
            @Param("credentialId") Long credentialId, 
            @Param("userId") Long userId);

    /**
     * Find all deleted credentials for a user
     * Used to display trash/recycle bin
     * 
     * @param userId the ID of the user
     * @return List of deleted credentials
     */
    @Query("SELECT c FROM Credential c WHERE c.userId = :userId AND c.deleted = true")
    List<Credential> findDeletedByUserId(@Param("userId") Long userId);

    /**
     * Find credential by ID and user ID excluding deleted credentials
     * Used for normal operations that should not access deleted items
     * 
     * @param credentialId the ID of the credential
     * @param userId the ID of the user
     * @return Optional containing the credential if found and not deleted
     */
    @Query("SELECT c FROM Credential c WHERE c.credentialId = :credentialId AND c.userId = :userId AND c.deleted = false")
    Optional<Credential> findByCredentialIdAndUserIdAndDeletedFalse(
            @Param("credentialId") Long credentialId, 
            @Param("userId") Long userId);
}
